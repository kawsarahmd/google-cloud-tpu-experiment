# This code is adapted from the Hugging Face JAX examples section and customized for my specific use case.
...
class TrainingArguments:
    ...
    push_to_hub_final_step: bool = field(
        default=False, metadata={"help": "Whether or not to upload the trained model to the model hub after final stage of training."}
    )
...
class DataTrainingArguments:
    ...
    streaming: bool = field(
        default=False,
        metadata={"help": "Whether to use streaming mode for datasets to avoid downloading all data locally."},
    )
    max_train_steps: Optional[int] = field(
        default=None,
        metadata={"help": "Maximum number of training steps for streaming mode. If not set, will use estimated value."},
    )
    estimated_dataset_size: Optional[int] = field(
        default=100000,
        metadata={"help": "Estimated dataset size for streaming mode to calculate training steps."},
    )
...
def collect_batch_samples(dataset_iter, batch_size):
    """Collect samples from streaming dataset iterator to form a batch."""
    samples = []
    try:
...
def main():
    ...
    # Get the datasets: Modified for streaming support
    if data_args.dataset_name is not None:
        # Downloading and loading a dataset from the hub with streaming support
    ...
    if model_args.config_name:
        ...
        logger.warning("You are instantiating from scratch.")
    elif model_args.model_name_or_path:
        config = T5Config.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=model_args.cache_dir,
        ...
        logger.warning("You are instantiating from existing model.")
    else:
        config = CONFIG_MAPPING[model_args.model_type]()
        logger.warning("You are instantiating a new config instance from scratch.")

    # Preprocessing the datasets - modified for streaming
    if training_args.do_train:
        column_names = list(datasets["train"].features.keys()) if data_args.streaming else datasets["train"].column_names
    else:
        column_names = list(datasets["validation"].features.keys()) if data_args.streaming else datasets["validation"].column_names
    ...
    tokenized_datasets = datasets.map(
        ...
        load_from_cache_file=not data_args.overwrite_cache if not data_args.streaming else False,
    )
    ...
    if model_args.model_name_or_path:
        ...
        logger.warning(
                f"Creating model from existing model using model_name_or_path!!!"
            )
        ...
        logger.warning(f"Creating model from scratch using fresh config file!!!")
    ...
    # Store some constants - modified for streaming
    num_epochs = int(training_args.num_train_epochs)
    train_batch_size = int(training_args.per_device_train_batch_size) * jax.device_count()
    per_device_eval_batch_size = int(training_args.per_device_eval_batch_size)
    eval_batch_size = per_device_eval_batch_size * jax.device_count()

    # Calculate num_train_steps differently for streaming vs non-streaming
    if data_args.streaming:
        if data_args.max_train_steps is not None:
            num_train_steps = data_args.max_train_steps
        else:
            # Estimate based on dataset size
            estimated_dataset_size = data_args.estimated_dataset_size
            num_train_steps = estimated_dataset_size // train_batch_size * num_epochs
        logger.info(f"Using streaming mode with estimated {num_train_steps} training steps")
    else:
        num_train_steps = len(tokenized_datasets["train"]) // train_batch_size * num_epochs
        logger.info(f"Using non-streaming mode with {num_train_steps} training steps from {len(tokenized_datasets['train'])} samples")
