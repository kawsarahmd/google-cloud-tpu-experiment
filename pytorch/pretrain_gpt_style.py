#!/usr/bin/env python
"""
Universal GPT-Style Pretraining Script
Supports: GPT-2, GPT-Neo, GPT-J, LLaMA, OPT, BLOOM

Uses built-in HuggingFace DataCollatorForLanguageModeling for causal LM.
...
@dataclass
class Arguments:
    """All arguments for GPT-style pretraining."""

    # Model arguments
    model_name_or_path: str = field(
        metadata={"help": "Model checkpoint (e.g., gpt2, EleutherAI/gpt-neo-125M, meta-llama/Llama-2-7b-hf)"}
    )
    ...
    max_seq_length: int = field(
        default=1024, metadata={"help": "Maximum sequence length (GPT models typically use longer contexts)"}
    )

    # Training arguments
    output_dir: str = field(default="./output", metadata={"help": "Output directory"})
    per_device_train_batch_size: int = field(
        default=4, metadata={"help": "Batch size per device (smaller for larger models)"}
    )
    per_device_eval_batch_size: int = field(default=4, metadata={"help": "Eval batch size"})
    ...
    gradient_accumulation_steps: int = field(
        default=4, metadata={"help": "Gradient accumulation (helps with large models)"}
    )
...
def main():
    ...
    # Set pad token (GPT models don't have pad token by default)
    ...
    # ✅ Built-in HuggingFace Data Collator for Causal LM
