#!/usr/bin/env python
# This code is adapted from the Hugging Face JAX examples section and converted to PyTorch for TPU usage.
...
# PyTorch XLA imports for TPU support
import torch_xla
...
class T5MLMDataset(Dataset):
    """Simple dataset wrapper for tokenized datasets."""
...
def train_function(index, model_args, data_args, training_args):
    """
    Training function to be run on each TPU core.
    """
    # Get the TPU device
    ...
    if training_args.push_to_hub and is_master:
    ...
    if model_args.model_name_or_path:
        ...
        if is_master:
            logger.warning("Creating model from existing model using model_name_or_path!!!")
    else:
        config.vocab_size = len(tokenizer)
        model = T5ForConditionalGeneration(config)
        if is_master:
            logger.warning("Creating model from scratch using fresh config file!!!")
    ...
    # Create datasets
    train_dataset = T5MLMDataset(tokenized_datasets["train"])
    eval_dataset = T5MLMDataset(tokenized_datasets["validation"])
    ...
    for epoch in range(num_epochs):
        ...
        epoch_iterator = tqdm(train_dataloader, desc=f"Epoch {epoch}", disable=not is_master)
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % training_args.gradient_accumulation_steps == 0:
                ...
                if global_step % training_args.logging_steps == 0:
                    avg_loss = train_loss / training_args.logging_steps
                    learning_rate = lr_scheduler.get_last_lr()[0]

                    if is_master:
                        logger.info(f"Step {global_step}: Loss = {avg_loss:.4f}, LR = {learning_rate:.2e}")
                ...
                if training_args.eval_steps and global_step % training_args.eval_steps == 0:
                    ...
                    with torch.no_grad():
                        for eval_batch in tqdm(eval_dataloader, desc="Evaluating", disable=not is_master):
                ...
                if global_step % training_args.save_steps == 0:
                    ...
                    if is_master:
                        ...
                        if training_args.push_to_hub:
                            api.upload_folder(
                                commit_message=f"Saving weights and logs of step {global_step} with dataset {data_args.dataset_name}",
        ...
        xm.rendezvous("saving_epoch_checkpoint")
        if is_master:
    ...
    # Final save
    xm.rendezvous("saving_final_checkpoint")
    if is_master:
        ...
        if training_args.push_to_hub or training_args.push_to_hub_final_step:
            api.upload_folder(
                commit_message=f"Saving model of final step {global_step} with dataset {data_args.dataset_name}",
                folder_path=training_args.output_dir,
                repo_id=repo_id,
                repo_type="model",
                token=training_args.hub_token,
    ...
    if training_args.do_eval:
        ...
        with torch.no_grad():
            for eval_batch in tqdm(eval_dataloader, desc="Final Evaluation", disable=not is_master):
                eval_batch = {k: v.to(device) for k, v in eval_batch.items()}
                outputs = model(**eval_batch)
                eval_loss += outputs.loss.item()
                eval_steps += 1
        ...
        if is_master:
            logger.info(f"Final Eval Loss = {avg_eval_loss:.4f}")

            eval_results = {"eval_loss": avg_eval_loss}
...
def main():
    ...
    xmp.spawn(train_function, args=(model_args, data_args, training_args), nprocs=None, start_method='fork')
